...
impl<const N: usize> ExecutionRouter<N> {
    ...
    pub fn is_interactive(cmd: &str) -> bool {
        ...
        matches!(
            program,
            "htop" | "top" | "less" | "more" | "ssh" | "vim" | "vi" | "nvim" | "nano" | "tmux" | "screen"
        )
    }
    
    fn push_block(&self, block: TypedBlock) {
        while let Err(_) = self.producer.try_push(block.clone()) {
            std::hint::spin_loop();
        }
    }
    ...
    pub async fn execute(&self, cmd: &str, rows: u16, cols: u16) -> Result<bool, Box<dyn std::error::Error>> {
        if Self::is_interactive(cmd) {
            ...
            let mut stderr = child.stderr.take().expect("stderr failed");
                ...
                loop {
                    match stdout.read(&mut buf).await {
                        ...
                        Ok(n) => {
                            ...
                            while let Err(_) = producer_out.try_push(block.clone()) {
                                std::hint::spin_loop();
                            }
                        }
                        Err(_) => break,
                ...
                loop {
                    match stderr.read(&mut buf).await {
                        ...
                        Ok(n) => {
                            ...
                            while let Err(_) = producer_err.try_push(block.clone()) {
                                std::hint::spin_loop();
                            }
                        }
                        Err(_) => break,
            ...
            tokio::spawn(async move {
                ...
                let block = TypedBlock::ExitStatus(status.code().unwrap_or(-1));
                while let Err(_) = producer_exit.try_push(block.clone()) {
                    std::hint::spin_loop();
                }
            });
    ...
    pub async fn execute_sandboxed(&self, cmd: &str, args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
