...
impl KeyEncoder {
    /// Builds the exact byte payload to write to the PTY for pasted text.
    ///
    /// Terminal paste input is normalized to carriage returns because that is
    /// what interactive shells expect on a PTY. When bracketed paste is active,
    /// the payload is wrapped in xterm's bracketed paste delimiters.
...
mod tests {
    ...
    fn test_encode_legacy_shift_up() {
        ...
        assert_eq!(
            KeyEncoder::encode_legacy(&input),
            Some(vec![27, b'[', b'1', b';', b'2', b'A'])
        );
    }
    ...
    #[test]
    fn test_encode_paste_empty_text() {
        assert!(KeyEncoder::encode_paste("", false).is_empty());
        assert!(KeyEncoder::encode_paste("", true).is_empty());
    }

    #[test]
    fn test_encode_paste_normalizes_newlines() {
        assert_eq!(
            KeyEncoder::encode_paste("one\ntwo\r\nthree", false),
            b"one\rtwo\rthree"
        );
    }

    #[test]
    fn test_encode_paste_wraps_bracketed_payload() {
        assert_eq!(
            KeyEncoder::encode_paste("one\ntwo", true),
    ...
    #[test]
    fn test_encode_paste_preserves_existing_carriage_returns() {
        assert_eq!(KeyEncoder::encode_paste("one\rtwo", false), b"one\rtwo");
    }
}
