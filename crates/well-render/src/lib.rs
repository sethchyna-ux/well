...
impl CellInstance {
    pub fn new(x: u32, y: u32, ch: char, fg_color: u32, bg_color: u32) -> Self {
        ...
        Self::with_glyph_slot(x, y, slot_id, fg_color, bg_color, false)
    }
    ...
    fn with_glyph_slot(
        ...
        let emoji_flag = if is_emoji { 1 << 15 } else { 0 };
        Self {
            grid_position: [x, y],
            glyph_id_and_effects: (slot_id & 0x1FFF) | emoji_flag,
            packed_colors,
        }
    }

    pub fn glyph_slot(&self) -> u32 {
        self.glyph_id_and_effects & 0x1FFF
    }

    pub fn is_emoji(&self) -> bool {
        self.glyph_id_and_effects & (1 << 15) != 0
    }
}
...
fn is_renderable_combining_component(ch: char) -> bool {
    !matches!(ch, '\u{200D}' | '\u{FE0E}' | '\u{FE0F}')
}

fn rasterization_char(font: &fontdue::Font, requested: char) -> Option<char> {
    if font.lookup_glyph_index(requested) != 0 {
        Some(requested)
...
pub struct CellBuildOptions {
    pub max_rows: u32,
    pub max_cols: u32,
    pub cursor_visible: bool,
    pub cursor_style: u32,
    pub theme_id: u32,
    pub cursor_color_override: u32,
    pub selection: Option<SelectionRange>,
}

impl CellBuildOptions {
...
pub const SLOT_ASCII_STYLIZED_START: u32 = 99; // 95-98 are reserved for cursor/prompt glyphs
pub const SLOT_ASCII_STYLIZED_END: u32 = 2047;
...
const _: () = assert!(SLOT_ASCII_STYLIZED_START > 98);
const _: () = assert!(SLOT_ASCII_STYLIZED_START <= SLOT_ASCII_STYLIZED_END);
...
pub struct OrpheusRenderer {
    ...
    pub image_pipeline: Option<crate::pipeline::ImagePipeline>,
}
...
impl OrpheusRenderer {
    pub async fn new(
        ...
        let mut renderer = Self {
            ...
            image_pipeline: Some(crate::pipeline::ImagePipeline::new(&device, surface_format)),
        };
    ...
    pub fn resolve_or_allocate_glyph(
        ...
        let slot_id = if is_emoji {
            ...
            slot
        } else if (32..=126).contains(&codepoint) && style == 0 {
    ...
    fn rasterize_glyph_to_gpu(&self, codepoint: u32, _style: u32, coords: AtlasCoordinates) {
        ...
        if ch == '█' || codepoint == 0x2588 {
            ...
            if let Some(font) = &self.font {
                ...
                let rasterized_char = rasterization_char(font, ch);
                let (metrics, bitmap) = rasterized_char
                    .map(|rasterized_char| font.rasterize(rasterized_char, font_size))
                    .unwrap_or_default();
    ...
    // Single Draw-Call Renderer Execution with optional image overlays
    pub fn draw_frame(
        &self,
        ...
        self.draw_frame_with_images(view, encoder, cells, clear_color, &[]);
    }

    pub fn draw_frame_with_images(
        &self,
        view: &wgpu::TextureView,
        encoder: &mut wgpu::CommandEncoder,
        cells: &[CellInstance],
        clear_color: Option<wgpu::Color>,
        images: &[(&wgpu::BindGroup, crate::pipeline::ImageQuad)],
    ) {
        if !cells.is_empty() {
            let max_cells =
                (self.instance_buffer.size() / std::mem::size_of::<CellInstance>() as u64) as usize;
        ...
        // Render Kitty graphics protocol texture overlays
        if !images.is_empty() {
            if let Some(pipeline) = &self.image_pipeline {
                pipeline.draw_images(&mut render_pass, &self.queue, images);
            }
        }
    }
    ...
    pub fn build_cells_from_vt100(
        ...
        self.build_cells_from_vt100_ext(screen, cells, CellBuildOptions::new(max_rows, max_cols));
    }
    ...
    pub fn build_cells_from_vt100_ext(
        ...
        Self::build_cells_with_glyph_resolver(screen, cells, options, |ch, style| {
            let is_emoji = is_emoji_codepoint(ch);
            let coords = self.resolve_or_allocate_glyph(ch as u32, style, is_emoji);
            (coords.layer * 32 + coords.col, is_emoji)
        });
    }
    ...
    pub fn build_cells(
        ...
        Self::build_cells_ext(screen, cells, options);
    }
    ...
    pub fn build_cells_ext(
        ...
        Self::build_cells_with_glyph_resolver(screen, cells, options, |ch, _style| {
            let instance = CellInstance::new(0, 0, ch, 0, 0);
            (instance.glyph_slot(), false)
        });
    }

    fn build_cells_with_glyph_resolver(
        screen: &vt100::Screen,
        cells: &mut Vec<CellInstance>,
        options: CellBuildOptions,
        mut resolve_glyph: impl FnMut(char, u32) -> (u32, bool),
    ) {
        let CellBuildOptions {
            max_rows,
            max_cols,
            cursor_visible,
            cursor_style,
        ...
        for row in 0..rows {
            for col in 0..cols {
                if let Some(cell) = screen.cell(row as u16, col as u16) {
                    ...
                    let is_selected = selection.is_some_and(|s| s.contains(col as u16, row as u16));
                    ...
                    let style = u32::from(cell.bold()) | (u32::from(cell.italic()) << 1);

                    let mut glyphs = text
                        .chars()
                        .filter(|glyph| is_renderable_combining_component(*glyph))
                        .peekable();
                    if glyphs.peek().is_none() {
                        let (slot_id, is_emoji) = resolve_glyph(' ', style);
                        let mut instance =
                            CellInstance::with_glyph_slot(col, row, slot_id, fg, bg, is_emoji);
                        instance.glyph_id_and_effects |= effects << 13;
                        cells.push(instance);
                    }

                    for (glyph_index, glyph) in glyphs.enumerate() {
                        let (slot_id, is_emoji) = resolve_glyph(glyph, style);
                        let glyph_bg = if glyph_index == 0 { bg } else { 0x000 };
                        let mut instance = CellInstance::with_glyph_slot(
                            col, row, slot_id, fg, glyph_bg, is_emoji,
                        );
                        instance.glyph_id_and_effects |= effects << 13;
                        cells.push(instance);
                    }
            ...
            if cr < max_rows && cc < max_cols {
                ...
                match cursor_style {
                    1 => {
                        ...
                        let (slot_id, is_emoji) = resolve_glyph('│', 0);
                        cells.push(CellInstance::with_glyph_slot(
                            cc,
                            cr,
                            slot_id,
                            cursor_color,
                    ...
                    2 => {
                        ...
                        let (slot_id, is_emoji) = resolve_glyph(ch, 0);
                        let mut inst = CellInstance::with_glyph_slot(
                            cc,
                            cr,
                            slot_id,
                            cursor_color,
                    ...
                    _ => {
                        ...
                        if char_under != ' ' {
                            ...
                            let (slot_id, is_emoji) = resolve_glyph(char_under, 0);
                            cells.push(CellInstance::with_glyph_slot(
                                cc,
                                cr,
                                slot_id,
    ...
    match color {
        ...
        vt100::Color::Idx(idx) => {
            ...
            if (idx as usize) < ANSI_COLORS.len() {
                ...
                let c = idx - 16;
                const XTERM_LEVELS: [u32; 6] = [0x0, 0x5, 0x8, 0xA, 0xD, 0xF];
                let r = XTERM_LEVELS[(c / 36) as usize];
                let g = XTERM_LEVELS[((c % 36) / 6) as usize];
                let b = XTERM_LEVELS[(c % 6) as usize];
                (r << 8) | (g << 4) | b
            } else if idx >= 232 {
                let g4 = (u32::from(idx - 232) * 15) / 23;
                (g4 << 8) | (g4 << 4) | g4
            } else {
                default_color
            }
...
mod tests {
    ...
    fn test_cursor_styles_and_blinking() {
        ...
        OrpheusRenderer::build_cells(parser.screen(), &mut cells, CellBuildOptions::new(5, 20));
        ...
        let mut hidden_options = CellBuildOptions::new(5, 20);
        hidden_options.cursor_visible = false;
        OrpheusRenderer::build_cells(parser.screen(), &mut cells_hidden, hidden_options);
        assert_eq!(cells_hidden.len(), cells.len() - 1); // No cursor cell added!

        // 3. Beam cursor (1)
        let mut cells_beam = Vec::new();
        let mut beam_options = CellBuildOptions::new(5, 20);
        beam_options.cursor_style = 1;
        OrpheusRenderer::build_cells(parser.screen(), &mut cells_beam, beam_options);
        ...
        let mut underline_options = CellBuildOptions::new(5, 20);
        underline_options.cursor_style = 2;
        OrpheusRenderer::build_cells(parser.screen(), &mut cells_underline, underline_options);
    ...
    fn test_build_cells_with_selection() {
        ...
        OrpheusRenderer::build_cells_ext(
            ...
            CellBuildOptions {
                ...
                selection: Some(SelectionRange::new(0, 0, 4, 0)), // "hello"
                ..CellBuildOptions::new(5, 30)
            },
        );
        ...
        OrpheusRenderer::build_cells_ext(
            ...
            CellBuildOptions {
                cursor_visible: false,
                selection: Some(SelectionRange::new(15, 2, 25, 2)),
                ..CellBuildOptions::new(5, 30)
            },
        );
    ...
    #[test]
    fn test_dynamic_glyph_resolution_preserves_unicode_combining_and_emoji() {
        let mut parser = vt100::Parser::new(3, 20, 0);
        parser.process("A界e\u{301}😀".as_bytes());

        let mut resolved = Vec::new();
        let mut cells = Vec::new();
        let mut options = CellBuildOptions::new(3, 20);
        options.cursor_visible = false;
        OrpheusRenderer::build_cells_with_glyph_resolver(
            parser.screen(),
            &mut cells,
            options,
            |ch, style| {
                resolved.push((ch, style));
                (ch as u32 & 0x1FFF, is_emoji_codepoint(ch))
            },
        );

        assert_eq!(
            resolved.iter().map(|(ch, _)| *ch).collect::<Vec<_>>(),
            vec!['A', '界', 'e', '\u{301}', '😀']
        );
        assert_eq!(
            cells
                ...
                .collect::<Vec<_>>(),
            vec![[0, 0], [1, 0], [3, 0], [3, 0], [4, 0]]
        );
        assert_eq!(cells[2].packed_colors & 0xFFF, 0);
        assert_eq!(cells[3].packed_colors & 0xFFF, 0);
        assert!(cells.last().unwrap().is_emoji());
        assert_ne!(cells[1].glyph_slot(), 0);
    }

    #[test]
    fn test_bold_text_does_not_enable_strikethrough_effect() {
        let mut parser = vt100::Parser::new(2, 10, 0);
        parser.process(b"\x1b[1mB");
        let mut cells = Vec::new();
        let mut options = CellBuildOptions::new(2, 10);
        options.cursor_visible = false;
        OrpheusRenderer::build_cells(parser.screen(), &mut cells, options);

        assert_eq!(cells.len(), 1);
        assert_eq!(cells[0].glyph_id_and_effects & (1 << 14), 0);
    }

    #[test]
    fn test_xterm_color_cube_and_rgb_quantization() {
        assert_eq!(vt100_color_to_rgb(vt100::Color::Rgb(255, 128, 0), 0), 0xF80);
        assert_eq!(vt100_color_to_rgb(vt100::Color::Idx(16), 0), 0x000);
        assert_eq!(vt100_color_to_rgb(vt100::Color::Idx(21), 0), 0x00F);
        assert_eq!(vt100_color_to_rgb(vt100::Color::Idx(196), 0), 0xF00);
        assert_eq!(vt100_color_to_rgb(vt100::Color::Idx(231), 0), 0xFFF);
        assert_eq!(vt100_color_to_rgb(vt100::Color::Idx(255), 0), 0xFFF);
    }

    #[test]
    fn test_missing_unicode_uses_visible_fallback_glyph() {
        let font_data = include_bytes!("../../../assets/fonts/OpenDyslexicNerdFont-Regular.otf");
        let font = fontdue::Font::from_bytes(&font_data[..], fontdue::FontSettings::default())
            .expect("failed to parse renderer font");
        let unsupported = char::from_u32(0x10FFFF).unwrap();

        assert_eq!(font.lookup_glyph_index(unsupported), 0);
        assert_eq!(rasterization_char(&font, unsupported), Some('?'));
    }
