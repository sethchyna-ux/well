...
impl HephaestusEngine {
    ...
    pub fn upload_stream_chunk(&self, chunk_data: &[u8], patterns: &[&str], clear_histogram: bool) {
        ...
        let buffer_len_buffer = self.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let automaton_buffer = self.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        if clear_histogram {
            ...
            self.queue.write_buffer(&self.histogram_buffer, 0, bytemuck::cast_slice(&zero_histogram));
        }

        // Always clear bitmask for the current chunk
        let zero_bitmask = vec![0u32; BITMASK_U32_COUNT];
        self.queue.write_buffer(&self.bitmask_buffer, 0, bytemuck::cast_slice(&zero_bitmask));
...

        {
            ...
            let workgroups = (chunk_data.len() as u32 + 255) / 256;
        ...
        encoder.copy_buffer_to_buffer(
            &self.bitmask_buffer, 0,
            &self.bitmask_staging, 0,
