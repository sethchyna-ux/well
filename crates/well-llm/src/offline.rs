...
impl OfflineRuleEngine {
    ...
    pub fn translate(query: &str) -> Option<OfflineTranslation> {
        ...
        if let Some(caps) = Regex::new(r"(?:kill|terminate|stop)\s+(?:process\s+)?(?:on\s+)?port\s+(\d+)").ok()?.captures(&q) {
            let port = &caps[1];
        ...
        if let Some(caps) = Regex::new(r"(?:find|check|show|what is)\s+(?:using|on)\s+port\s+(\d+)").ok()?.captures(&q) {
            let port = &caps[1];
            return Some(OfflineTranslation {
        ...
        if q.contains("top processes by memory") || q.contains("processes using most memory") || q.contains("most ram") {
            return Some(OfflineTranslation {
                command: "ps aux --sort=-%mem | head -n 10".to_string(),
                explanation: "Lists top 10 processes consuming the highest resident memory.".to_string(),
                confidence: 0.95,
            });
        }

        if q.contains("top processes by cpu") || q.contains("processes using most cpu") || q.contains("highest cpu") {
            return Some(OfflineTranslation {
        ...
        // 2. Git Workflows
        if q.contains("revert last commit keep changes") || q.contains("undo commit keep staged") || q.contains("undo last commit keep changes") {
            return Some(OfflineTranslation {
        ...
        if q.contains("undo last commit completely") || q.contains("discard last commit") || q.contains("nuke last commit") {
            return Some(OfflineTranslation {
                command: "git reset --hard HEAD~1".to_string(),
                explanation: "Permanently discards the latest commit and all working tree changes.".to_string(),
                confidence: 0.99,
            });
        }

        if q.contains("discard all changes") || q.contains("discard unstaged") || q.contains("revert all changes") {
            return Some(OfflineTranslation {
                command: "git restore .".to_string(),
                explanation: "Discards all uncommitted changes in tracked files within the working tree.".to_string(),
                confidence: 0.97,
            });
        }

        if q.contains("clean untracked") || q.contains("delete untracked files") || q.contains("remove untracked") {
            return Some(OfflineTranslation {
                command: "git clean -fd".to_string(),
                explanation: "Recursively deletes untracked files and directories from the repository.".to_string(),
                confidence: 0.96,
            });
        }

        if let Some(caps) = Regex::new(r"(?:create|checkout|switch to)\s+(?:new\s+)?branch\s+([a-zA-Z0-9_\-\./]+)").ok()?.captures(&q) {
            let branch = &caps[1];
            return Some(OfflineTranslation {
        ...
        if q.contains("commit history graph") || q.contains("git graph") || q.contains("visualize commits") {
            return Some(OfflineTranslation {
                command: "git log --oneline --graph --decorate --all".to_string(),
                explanation: "Displays visual ASCII topology graph of commits across all branches.".to_string(),
                confidence: 0.96,
            });
        }
        ...
        if let Some(caps) = Regex::new(r"(?:find|search)\s+(?:files\s+)?larger\s+than\s+(\d+)\s*(mb|gb|kb|m|g|k)").ok()?.captures(&q) {
            let num = &caps[1];
        ...
        if q.contains("modified in last 24 hours") || q.contains("changed today") || q.contains("modified today") {
            return Some(OfflineTranslation {
        ...
        if let Some(caps) = Regex::new(r#"(?:search for|find)\s+(?:text\s+)?["']([^"']+)["']"#).ok()?.captures(&q) {
            let text = &caps[1];
            return Some(OfflineTranslation {
                command: format!("grep -rn \"{text}\" ."),
                explanation: format!("Recursively searches for '{text}' across all files with line numbers."),
                confidence: 0.94,
            });
        }
        ...
        if q.contains("folder size") || q.contains("directory size") || q.contains("how big is this folder") {
            return Some(OfflineTranslation {
        ...
        if let Some(caps) = Regex::new(r"(?:extract|unzip|untar)\s+([a-zA-Z0-9_\-\.]+\.(?:tar\.gz|tgz|tar))").ok()?.captures(&q) {
            let archive = &caps[1];
            return Some(OfflineTranslation {
                command: format!("tar -xzvf {archive}"),
                explanation: format!("Extracts contents of tarball '{archive}' into current working directory."),
                confidence: 0.97,
            });
        }
        ...
        if q.contains("list all containers") || q.contains("all docker containers") {
            return Some(OfflineTranslation {
                ...
                explanation: "Displays all Docker containers including stopped or exited instances.".to_string(),
                confidence: 0.98,
            });
        }
        ...
        if q.contains("docker clean") || q.contains("docker prune") || q.contains("clean docker images") {
            return Some(OfflineTranslation {
                command: "docker system prune -a --volumes".to_string(),
                explanation: "Removes unused containers, dangling networks, and unreferenced build cache.".to_string(),
                confidence: 0.95,
            });
        }
        ...
        if q.contains("check compilation") || q.contains("cargo check") {
            return Some(OfflineTranslation {
                ...
                explanation: "Rapidly validates code syntax and type checks without running code generator.".to_string(),
                confidence: 0.99,
            });
        }
        ...
        if q.contains("run tests") || q.contains("cargo test") {
            return Some(OfflineTranslation {
                ...
                explanation: "Runs all unit/integration tests with live stdout streaming.".to_string(),
                confidence: 0.98,
            });
        }
        ...
        if q.contains("my ip") || q.contains("public ip") || q.contains("what is my ip") {
            return Some(OfflineTranslation {
                ...
                explanation: "Queries external IP echo service over HTTPS to return public WAN IP.".to_string(),
                confidence: 0.97,
            });
        }

        if let Some(caps) = Regex::new(r"(?:ping|test connection to)\s+([a-zA-Z0-9_\-\.]+)").ok()?.captures(&q) {
            let host = &caps[1];
            return Some(OfflineTranslation {
                command: format!("ping -c 4 {host}"),
                explanation: format!("Sends 4 ICMP ECHO packets to test network reachability to {host}."),
                confidence: 0.98,
            });
        }
