...
impl GeminiClient {
    ...
    pub fn translate_query(
        ...
        let structured: StructuredShellOutput = serde_json::from_str(&raw_text)
            .map_err(|e| format!("Failed to parse structured output from Gemini: {} (raw: {})", e, raw_text))?;

        Ok(structured)
    }

    /// Generates an image from a textual prompt using Google Imagen 3 API.
    /// Returns the raw PNG bytes and a generated filename.
    pub fn generate_image(&self, prompt: &str, width: u32, height: u32) -> Result<(Vec<u8>, String), String> {
        let url = format!(
        ...
        let b64_bytes = json["candidates"][0]["content"]["parts"][0]["inlineData"]["data"]
            .as_str()
            .ok_or_else(|| "Missing base64 image payload in Gemini API response".to_string())?;
