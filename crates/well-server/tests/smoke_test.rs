...
fn test_well_server_headless_execution() {
    ...
    stdout.read_exact(&mut payload).expect("Failed to read payload from stdout");

    // Deserialize into TypedBlock
    let block: TypedBlock = bincode::deserialize(&payload).expect("Failed to deserialize TypedBlock");
    ...
    let content = match block {
        ...
        TypedBlock::StderrChunk(data) => {
            child.kill().unwrap();
            panic!("Expected StdoutChunk, got StderrChunk: {}", String::from_utf8_lossy(&data));
        }
        _ => {
            child.kill().unwrap();
    ...
    assert!(content.contains("headless well-server working"), "Output did not contain expected string, got: {}", content);
